...
def scrape_youtube_comments(video_url, max_comments=90):
    try:
            ...
            for comment in comment_elements:
                ...
                if author and text and timestamp:
                    collected_comments.append({
                        "video_id": video_url.split("v=")[-1].split("&")[0],  # Extract video ID more robustly
...
def main():
    parser = argparse.ArgumentParser(description="Scrape YouTube comments from multiple videos.")
    parser.add_argument("video_urls", nargs="+", help="List of YouTube video URLs to scrape")
