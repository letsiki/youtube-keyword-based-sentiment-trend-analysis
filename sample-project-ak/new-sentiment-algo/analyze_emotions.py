...
def analyze_emotion(comment):
    try:
        # Run the classifier
        results = emotion_classifier(comment)[0]  # [0] since pipeline returns a list of lists
...
def map_emotion_to_number(emotion_str):
    try:
        ...
        if "label" not in emotion_data:
            return None

        label = emotion_data["label"].lower()  # lowercase from model
